...
results is shown in Table~\ref{tab:results-summary}. Table \ref{tab:legend} describes the acronyms used in the figures.
...
\begin{table}
    ...
    \begin{tabular}{c|c}
        ...
        \hline
        \textbf{AP} & Approximating Pipeline \\
        \textbf{BP} & Baseline Pipeline \\
        \textbf{CSP} & Cluster Simplify Pipeline \\
        \textbf{PTP} & Phonotactics Pipeline \\
        \textbf{TKP} & Toki Pona Pipeline \\
        \textbf{IP} & Isolating Pipeline \\
        \textbf{MTP} & Model Temperature Pipeline \\
        \textbf{TLP} & Two Level Pipeline \\
    \end{tabular}
    \caption{Legend for result figures}
    \label{tab:legend}
\end{table}
